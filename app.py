...
#importing data
data = pd.read_csv('soccerDataset.csv')
...
def generate_plots():
    ...
    axs[2].plot(x, y3, color='green', label='tan(x)')
...
navbar = dbc.Navbar(
    [
        html.A(
            dbc.Row(
                [
                    dbc.Col(html.Img(src='/assets/foot2.png', height="150px"), width=3, style={'textAlign': 'left'}),
                    dbc.Col(
                        [
                            html.Label("SOCCER ATHELETE ANALYSIS DASHBOARD", id="label1"),
                            html.Label("Analysing all generation soccer athletes", className="label2"),
                            html.Br(),
                            html.Label("Data Visualization project by: Suyash Bansal (B20CS076) and Sudip Kumar (B20CS072)", className="label2", style={'margin-bottom': '.24rem'})
                        ],
                        width=6,
                        style={'textAlign': 'center'}
                    ),
                    dbc.Col(html.Img(src='/assets/player_image.png', height="150px", style={'borderRadius': '10px', 'margin-top': '.54rem'}), width=3, style={'textAlign': 'right'})
                ],
                align="between",
                # no_gutters=True,  # Remove gutters between columns
            ),
        ),
    ],
)
...
controls_club = dbc.Card(
    [
        ...
        dbc.Form(
            [
                html.Label('Select an Aspect for x:'),
                html.Br(),
        ...
        dbc.Form(
            [
                html.Label('Select an Aspect for y:'),
                html.Br(),
    ...
    [
        dbc.Card(
            dbc.CardBody([
                    ...
                    html.H1('Athelete Evaluation', style={'textAlign': 'center'}),
                    html.Hr(),

                    dbc.Row([
                        dbc.Col([
                            dbc.Row(controls_player_1),
                            dbc.Row(html.Img(src='/assets/player2.png',className="playerImg"))
                        ],sm=3),
                        dbc.Col(dcc.Graph(id='graph_example'), sm=5, align = 'center'),
                        dbc.Col([
                            dbc.Row(controls_player_2),
                            dbc.Row(html.Img(src='assets\player1.png',className="playerImg"))
                        ],sm=3),
                    ],justify="between"),
                    dbc.Row([
                        dbc.Col(
...
tab2_content = html.Div(
    [
        dbc.Card(    
            dbc.CardBody(
                [
                    html.H1("Tournament Analysis", style={'textAlign': 'center'}),
                    html.Hr(),
                    dbc.Row(
                        [
        ...
        dbc.Card(    
            dbc.CardBody(
                [
                    html.H1("Franchise performance Analysis", style={'textAlign': 'center'}),
                    html.Hr(),  
                    dbc.Row(
                        [
...
app.layout = dbc.Container([
        ...
        dbc.Tabs(
            [
                dbc.Tab(tab1_content, label="Athelete parity"),
                dbc.Tab(tab2_content, label="Tournament and franchise Parity"),
            ], 
        ),
    ],
    fluid=True,
...
def bar_plot(input_value1,input_value2,input_value3, age):
    ...
    yi = filtered_by_age_data[[input_value1 ,'league_name']]
    zi = yi.groupby('league_name').median()[input_value1].sort_values(ascending=False).head(5)
    data_bar1 = dict(
        type='bar',
        y=zi,
        x=filtered_by_age_data['league_name'].unique()
    )
    yi = filtered_by_age_data[[input_value2 ,'league_name']]
    zi = yi.groupby('league_name').median()[input_value2].sort_values(ascending=False).head(5)
    ...
    yi = filtered_by_age_data[[input_value3 ,'league_name']]
    zi = yi.groupby('league_name').median()[input_value3].sort_values(ascending=False).head(5)
    ...
    fig1.update_traces(marker_color='cyan', marker_line_color='white', marker_line_width=0.8, opacity=0.9)
    fig1.update_layout(title_text=input_value1.capitalize(),title_x=0.5,
                       margin=dict(l=70, r=40, t=60, b=40),
                       plot_bgcolor = 'rgba(0, 0, 0, 0)',
                       paper_bgcolor = 'rgba(0, 0, 0, 0)',
                       font=dict(color='white'),
                       xaxis = dict(gridcolor= "#e5e6dc",gridwidth= 0.5, tickfont=dict(color='white')),
                       yaxis = dict(tickcolor= "#e5e6dc",tickwidth= 5,gridcolor= "#e5e6dc",gridwidth= 0.5, tickfont=dict(color='white')))
    
    fig2 = go.Figure(data=data_bar2, layout=layout_bar2)
    fig2.update_traces(marker_color='rgb(255,171,0)', marker_line_color='rgb(255,171,0)', marker_line_width=0.8, opacity=0.9)
    ...
    fig3.update_traces(marker_color='cyan', marker_line_color='white', marker_line_width=0.8, opacity=0.9)
...
def plots_clubs(league,x_val,y_val):
    ...
    fig1 = px.scatter(data_frame  = plot_df, x=x_val, y=y_val,color="isOver25text", size = 'potential',
                        color_discrete_sequence=['cyan','rgb(255,171,0)'], hover_name ='short_name',
    ...
    fig2 = go.Figure()
    fig2.add_trace(go.Bar(y=x,x=y[True],name='Over 25',orientation='h', marker=dict(color='cyan',line=dict(color='DarkSlateGrey', width=1.1))))
    ...
    fig2.update_yaxes(tickfont = dict(size=10, color='white'))
    fig2.update_layout(barmode='stack', 
                        title = dict(text ='Players over 25 and under 25 years old by club',
                                     font = dict(size =14, color='white')),
                        plot_bgcolor = 'rgba(0, 0, 0, 0)',
                        paper_bgcolor = 'rgba(0, 0, 0, 0)',    
                        legend=dict(title = dict(text ="Age of players", font = dict(size =14, color='white')),
                                        orientation="h",
                                        yanchor="bottom",
                                        y=1,
                                        xanchor="right",
                                        x=0.95),font=dict(color='white'),
...
def tab_1_function(player1, player2):
    ...
    barplot1.update_traces(marker_color='cyan')
